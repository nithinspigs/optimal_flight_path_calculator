...
REPO_NAME := optimal_flight_path_calculator
PACKAGE_NAME := optimal_flight_path_calculator

cwd := $(shell pwd)
init_hook := "import os, sys; sys.path.append(\"${cwd}/src/${PACKAGE_NAME}\")"

validate:
	venv/dev_${REPO_NAME}/bin/pylint --init-hook=${init_hook} -r n --output-format=parseable --msg-template="{path}:{line}: [{msg_id}({symbol}), {obj}] {msg}" --exit-zero --ignore-paths="src/${PACKAGE_NAME}/bak" src/${PACKAGE_NAME} tests | tee validation/pylint-report.txt
...
venv/dev_${REPO_NAME}: ## create virtual environment if venv is not present
	python -m venv venv/dev_${REPO_NAME}
...
venv/test_${REPO_NAME}: ## create virtual environment if venv is not present
	python -m venv venv/test_${REPO_NAME}
